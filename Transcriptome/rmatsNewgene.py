...
#      based on the stringtie new merged reference genome.
#Example:python 05rmatsOldgene.py ~/Hisat
#        python 05rmatsOldgene.py <Bam file after alighment>
...
ref = os.path.abspath('stringtie_merged.gtf')

os.chdir('../')
if not(os.path.isdir('new_Rmats')):
    os.mkdir('new_Rmats')
os.chdir('./new_Rmats')
for k in dic:
    f = open(k+'.txt','w+')
